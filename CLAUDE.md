...
- `endGame()` cancels the animation frame, but when called from within `loop` (auto-drop lock), `loop` schedules a new frame right after, so the loop keeps running behind the Game Over overlay.
- Resuming from pause (`togglePause`) does not re-add the `hidden` class to the overlay.
